...
```
District `r highlight(q1_answer$CounDist)` has the most trees with `r highlight(format(q1_answer$n_trees, big.mark = ","))` trees.
...
District `r highlight(q2_answer$CounDist)` has the highest tree density — `r highlight(fmt_num(q2_answer$trees_per_sqmi, 1))` trees/sq mi (`r highlight(fmt_num(q2_answer$trees_per_acre, 2))` trees/acre).
...
```
District `r highlight(q3_answer$CounDist)` has the highest fraction of dead trees — `r highlight(fmt_num(100 * q3_answer$frac_dead, 2))`% (`r highlight(fmt_int(q3_answer$dead_trees))` of `r highlight(fmt_int(q3_answer$total_trees))`).
