...
ggplot() +
  ...
  labs(
    ...
    subtitle = if (plot_all) {
      ...
      paste(
        "Sample shown:", format(nrow(trees_to_plot), big.mark = ","), "of",
...
District `r highlight(q1_answer$CounDist)` has the most trees, with `r highlight(fmt_int(q1_answer$n_trees))` trees.
...
District `r highlight(q2_answer$CounDist[[1]])` has the highest tree density , with `r highlight(fmt_num(q2_answer$trees_per_sqmi[[1]], 1))` trees/sq mi
(`r highlight(fmt_num(q2_answer$trees_per_acre[[1]], 2))` trees/acre).
...
District `r highlight(q3_answer$CounDist[[1]])` has the highest fraction of dead trees, with `r highlight(fmt_num(100 * as.numeric(q3_answer$frac_dead[[1]]), 2))`%
...
The most common tree species in Manhattan is `r highlight(q4_species)` with
`r highlight(fmt_int(q4_answer$n[[1]]))` trees.
...
if (!exists("closest_tree_wgs") || !exists("dist_lab_ft") || !exists("species_label")) {
  ...
  species_cols <- intersect(
    c("spc_common", "genusspecies", "spc_latin"),
...
leaflet() |>
  ...
  # nearby trees
  addCircleMarkers(
    data = trees_local, radius = 2, color = "#666",
  ...
  # Baruch label
  addMarkers(
    data = baruch_pt, label = "Baruch College (55 Lexington Ave)",
...
The closest tree to Baruch College is a `r highlight(q5_answer$species[[1]])` located
`r highlight(scales::comma(q5_answer$distance_ft[[1]], 1))` ft
(`r highlight(sprintf("%.1f", q5_answer$distance_m[[1]]))` m) away.
...
if (!exists("trees_with_districts")) {
  if (exists("TREES_CLEAN")) {
    ...
    stop("Need TREES_CLEAN or trees_with_districts")
  }
}
