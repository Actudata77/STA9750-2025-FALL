...
The longest film to have ever appeared in the Netflix global Top 10 is *`r longest_film$show_title`* with a runtime of *`r round(60 * longest_film$runtime)`* minutes.

**4.For each of the four categories, what program has the most total hours of global viewership?**

```{r}
top_viewed_by_category <- GLOBAL_TOP_10 |>
  group_by(category) |>
  slice_max(weekly_hours_viewed, n = 1) |>
  select(category, show_title, weekly_hours_viewed) |> 
  format_titles() |>
  datatable(options=list(searching=FALSE, info=FALSE)) |>
  formatRound(c('Weekly Hours Viewed'))
top_viewed_by_category
```
...
longest_tv_show_run <- COUNTRY_TOP_10 |>
  filter(category == "TV") |>  # or: category %in% c("TV (English)", "TV (Non-English)")
  group_by(country_name, show_title) |>          # include the show
  summarise(weeks = max(cumulative_weeks_in_top_10, na.rm = TRUE)) |>
  ungroup() |> 
  arrange(desc(weeks)) |>
  slice(1)
longest_tv_show_run
...
The TV show with the longest run in a country's Top 10 is *`r longest_tv_show_run$show_title`* in *`r longest_tv_show_run$country_name`* with a total of *`r longest_tv_show_run$weeks`* weeks.
...
last_country
```
The country where Netflix ceased operations the earliest is *`r last_country$country_name`* in the week of *`r last_country$max_week`*.
...
```
The total viewership of the TV show Squid Game across all seasons is *`r round(squid_game_viewership$total_hours)`* hours.
...
The movie Red Notice received approximately *`r round(red_notice_views_2021$total_views)`* views in 2021.
