...
![](images/jobsreport.png){width="100%" height="auto" fig-align="center"}
...
This mini-project investigates whether recent CES revisions are unusually large or politically concerning. To answer this question, I use modern data-acquisition techniques (`httr2`, `rvest`), multi-table data processing (`dplyr`), visualization (`ggplot2`), and statistical inference (`infer`) to construct a reproducible analysis pipeline. By re-creating the BLS’s reported CES levels and revisions directly from the agency’s website, I can explore long-run revision patterns and evaluate some factual claims

Throughout this project, I aim to combine technical accuracy with transparent communication. My goal is to understand *how* CES estimates change, *why* revisions occur, and whether the data supports or contradicts public claims about BLS reliability.

## Data Acquisition and Preparation

To evaluate claims about **CES** accuracy, I first need two foundational datasets:

- **Final CES employment levels** – the seasonally adjusted **total nonfarm payroll** series that provides the headline jobs number reported each month by the **BLS**.
- **Cycle-to-cycle CES revisions** – tables that track how the first estimate of **total nonfarm payroll** is updated in subsequent releases, allowing us to quantify the size and direction of **revisions** over time.
...
We now have a complete dataset covering more than 45 years of employment reporting. By joining the two sources into a single `ces` table, and can explore:
...
With both the final **CES** employment levels and the month-to-month **revisions** in hand, I now explore how these series behave over the 1979–2025 window. In this section I:
...
(t statictic = –0.70, p-value = 0.76, CI: –49k to 21k).
...
```
The correlation between monthly CES employment change and the absolute size of CES revisions is negative and statistically significant (r = −0.143, p < 0.01). This indicates that months with larger employment changes tend to have slightly smaller revisions, though the relationship is weak in magnitude ( 0.14)
...
- **Largest downward revision:** **−537k** (also related to the pandemic era)  
...
### Fact-Check #2  
*“BLS job numbers are manipulated during election years”*
...
Finally, applying these results to real political claims showed that popular narratives about CES bias or partisan influence are not supported by empirical evidence. While revisions are unavoidable in a large, they do not display long-term directional or political patterns.
