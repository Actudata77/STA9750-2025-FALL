...
ces_tbl_list <- ces_page |> html_elements("table") |> html_table()
...
# Cleaned CES levels — head
nice_table(head(ces_levels, 6),
           caption = "Cleaned CES Levels — First 6 Rows")
...
# Take 47 consecutive tables: 1979–2025.
...
tiny_table(
  ...
  caption = "Largest Positive CES Revision (1979–2025)"
)

tiny_table(
  largest_neg |> rename_with(stringr::str_to_title),
  caption = "Largest Negative CES Revision (1979–2025)"
)
...
nice_table(
  mean_test_tbl,
  caption = "Test of Whether the Average CES Revision Differs from Zero (1979–2025)",
  page_len = 5
)
...
A one-sample t-test finds this mean significantly greater than zero (t = 3.26, p ≈ 0.001).
...
### Task 4.5 – Are revisions larger when the underlying CES change is larger?
...
```
The correlation between monthly CES employment change and the absolute size of CES revisions is negative and statistically significant (r = −0.143, p < 0.01). This indicates that months with larger employment changes tend to have slightly smaller revisions, though the relationship is weak in magnitude.

## Fact-Check BLS Revisions

### Fact-Check #1  
**“The BLS always underestimates job growth and then revises the numbers upward.”**

This claim appears frequently in commentary and media discussions. It implies that BLS initial estimates are *systematically too low*, meaning revisions should consistently be positive. With our full dataset of CES revisions (1979–2025), we test this claim directly.
...
- **t-statistic:** 3.26  
- **p-value:** 0.001 (statistically significant, but…)  

**Interpretation:**  
Even though the mean is slightly positive, the confidence interval *crosses zero*, indicating that revisions vary widely and we cannot conclude a persistent upward bias.

---

### **Supporting Evidence (Tasks 2–3)**

- **Mean revision:** +11.5k  
- **Median revision:** +4k  
- **Fraction of positive revisions:** 52.8% (barely above pure random ~50%)  
- **Largest upward revision:** +345k (pandemic; outlier)  
- **Largest downward revision:** –537k (pandemic; outlier)  
- **Decade medians:** hover near zero  
- **Revision time-series:** swings above/below zero with no long-run trend  
...
- No decade shows a **systematic upward bias**.  
- Extreme revisions correspond to **macroeconomic shocks**, not systematic underestimation.  
- Statistical evidence shows **no persistent revision bias**.
...
This narrative resurfaces every election cycle. To evaluate it, we compare CES revision magnitudes in **presidential election years vs. non-election years**.

Election years since 1979:  
1980, 1984, 1988, 1992, 1996, 2000, 2004, 2008, 2012, 2016, 2020, 2024.

We classify each observation:

- `election_year = TRUE` if the month falls in an election calendar year  
- Otherwise `FALSE`
...
Because alleged “manipulation” would imply **larger** changes, we compare **absolute** revisions.
...
(This is a one-sided test of increased volatility in election years.)
...
**Interpretation:**  
The confidence interval spans zero, indicating *no evidence* that revisions are systematically larger during election years.
...
- **Mean revision (non-election years):** +11.9k  
- **Median absolute revisions:** nearly identical  
- **Major spikes:** occur in 2020 due to **COVID-19**, not the election  
- **Decade analysis:** no election-year anomalies  

---

### **Conclusion**

There is **no statistical evidence** that CES revisions are manipulated or behave differently in presidential election years.

Revisions during election years:

- are **not meaningfully larger**,  
- are **not more positive or more negative**,  
- and are **dominated by macroeconomic shocks**, not political cycles.
