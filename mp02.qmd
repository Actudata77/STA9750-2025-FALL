...
# Step 1: Calculate average household size over time for each CBSA
household_size_data <- POPULATION |>
  inner_join(HOUSEHOLDS, by = c("GEOID", "NAME", "year")) |>
  mutate(avg_household_size = population / households)

# Step 2: Create the spaghetti plot colored by CBSA
ggplot(household_size_data, aes(x = year, y = avg_household_size, group = GEOID, color = GEOID)) +
  ...
  theme(
    ...
    legend.position = "none"  # Too many CBSAs to show legend
  )
```
...
When visualizing the evolution of many units over time, overlapping line plots often become quite difficult to read, resulting in "spaghetti plots." A useful strategy is to highlight a small number of important lines while 'low-lighting' the other lines so that they only serve as a background of 'representative samples'. The `gghighlight` package provides tools for making this sort of plot.

For this extra credit, I use the `gghighlight` package to highlight the NYC and Los Angeles CBSAs in the visualization of household size over time.
...
# Step 2: Create indicator for cities to highlight (NYC and LA)
household_size_data <- household_size_data |>
  mutate(
    highlight_city = grepl("New York|Los Angeles", NAME, ignore.case = TRUE)
  )

# Step 3: Create the highlighted spaghetti plot
ggplot(household_size_data, aes(x = year, y = avg_household_size, group = GEOID)) +
  geom_line(alpha = 0.6, linewidth = 0.8) +
  gghighlight(
    highlight_city,                    # Condition: which cities to highlight
    use_direct_label = TRUE,           # Add city name labels directly on plot
    label_key = NAME,                  # Use NAME column for labels
    unhighlighted_params = list(       # Style for background (non-highlighted) lines
      colour = alpha("gray70", 0.3),
      linewidth = 0.5
    )
  ) +
  scale_y_continuous(
  ...
  labs(
    title = "Evolution of Average Household Size Over Time (2009-2023)",
    subtitle = "New York and Los Angeles CBSAs highlighted; other metros shown in gray",
    x = "Year",
    y = "Average Household Size (persons per household)",
    caption = "Source: American Community Survey (ACS)\nNote: 2020 data unavailable due to COVID-19"
  ) +
