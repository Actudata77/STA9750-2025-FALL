...
instantaneous_top <- housing_data |>
  ...
  datatable(instantaneous_top, 
          ...
          options = list(paging = TRUE, pageLength = 10, lengthChange = TRUE, searching = FALSE, info = TRUE)) |>
...
if (nrow(perfect_yimby) > 0) {
  perfect_yimby |> 
    ...
    format_titles() |> 
  datatable(perfect_yimby,
          caption = "CBSAs Meeting All 4 YIMBY Criteria (Perfect YIMBY Success):",
          options = list(paging = TRUE, pageLength = 10, lengthChange = TRUE, searching = TRUE, info = TRUE)) |> 
...
Politicians and local leaders always want to attract younger citizens. To make our YIMBY policy brief more appealing to younger demographics, we'll incorporate variables that specifically resonate with millennials and young professionals. We'll add two key metrics:

1. **Young Adult Population (Ages 25-34)**: This age group represents millennials and young professionals who are particularly affected by housing affordability
2. **Arts, Entertainment & Recreation Employment**: This sector (NAICS 71) represents the cultural amenities and lifestyle factors that attract young people to cities

```{r extra_credit_03_millennial_data, warning=FALSE, message=FALSE}
library(dplyr)
library(tidycensus)
library(purrr)

# Get young adult population data (ages 25-34)
# This requires downloading additional ACS data
get_young_adults <- function(start_year = 2009, end_year = 2023) {
  fname <- file.path("data", "mp02", "young_adults_2009_2023.csv")
  ...
  if(!file.exists(fname)) {
    ...
    ALL_DATA <- map(YEARS, function(yy){
      tidycensus::get_acs("cbsa", "B01001_007", year=yy, survey="acs1") |>  # Males 25-29
        rename(males_25_29 = estimate) |>
        select(GEOID, NAME, males_25_29) |>
        left_join(
          tidycensus::get_acs("cbsa", "B01001_008", year=yy, survey="acs1") |>  # Males 30-34
            rename(males_30_34 = estimate) |>
            select(GEOID, males_30_34),
          by = "GEOID"
        ) |>
        left_join(
          tidycensus::get_acs("cbsa", "B01001_031", year=yy, survey="acs1") |>  # Females 25-29
            rename(females_25_29 = estimate) |>
            select(GEOID, females_25_29),
          by = "GEOID"
        ) |>
        left_join(
          tidycensus::get_acs("cbsa", "B01001_032", year=yy, survey="acs1") |>  # Females 30-34
            rename(females_30_34 = estimate) |>
            select(GEOID, females_30_34),
          by = "GEOID"
        ) |>
        mutate(
          year = yy,
          young_adults_25_34 = males_25_29 + males_30_34 + females_25_29 + females_30_34
        ) |>
        select(GEOID, NAME, year, young_adults_25_34)
    }) |> bind_rows()
...
# Get arts, entertainment, and recreation employment from BLS WAGES data
ARTS_EMPLOYMENT <- WAGES |>
  filter(INDUSTRY == 71) |>  # NAICS 71: Arts, Entertainment, and Recreation
  mutate(std_cbsa = paste0(FIPS, "0")) |>
  group_by(std_cbsa, YEAR) |>
  summarise(
    arts_employment = sum(EMPLOYMENT, na.rm = TRUE),
    arts_avg_wage = weighted.mean(AVG_WAGE, EMPLOYMENT, na.rm = TRUE),
    .groups = "drop"
  )

# Join with POPULATION data to get CBSA names and calculate percentages
MILLENNIAL_DATA <- POPULATION |>
  ...
  mutate(
    young_adult_pct = (young_adults_25_34 / population) * 100,
    arts_emp_per_1000 = (arts_employment / population) * 1000
  )

# Calculate summary statistics for most recent year
recent_year <- max(MILLENNIAL_DATA$year, na.rm = TRUE)

millennial_summary <- MILLENNIAL_DATA |>
  filter(year == recent_year) |>
  select(NAME, population, young_adults_25_34, young_adult_pct, 
         arts_employment, arts_emp_per_1000) |>
  arrange(desc(young_adult_pct))

cat("Top 10 CBSAs by Young Adult Percentage (Ages 25-34):\n")
print(
  millennial_summary |>
    head(10) |>
    format_titles() |>
    datatable(options = list(searching = FALSE, info = FALSE, paging = FALSE)) |>
    formatRound(c("Population", "Young Adults 25 34"), 0) |>
    formatRound(c("Young Adult Pct", "Arts Emp Per 1000"), 2) |>
    formatRound(c("Arts Employment"), 0)
)

cat("\n\nTop 10 CBSAs by Arts, Entertainment & Recreation Employment per 1,000 residents:\n")
print(
  millennial_summary |>
    arrange(desc(arts_emp_per_1000)) |>
    head(10) |>
    format_titles() |>
    datatable(options = list(searching = FALSE, info = FALSE, paging = FALSE)) |>
    formatRound(c("Population", "Young Adults 25 34"), 0) |>
...
Now let's incorporate these millennial-friendly metrics into our YIMBY analysis:

```{r extra_credit_03_millennial_yimby, warning=FALSE, message=FALSE}
library(dplyr)
library(ggplot2)


# Merge millennial data with YIMBY criteria
yimby_millennial <- yimby_criteria |>
  left_join(
    MILLENNIAL_DATA |>
      filter(year == recent_year) |>
      select(GEOID, young_adult_pct, arts_emp_per_1000),
    by = "GEOID"
  )

# Create visualization showing relationship between YIMBY success and millennial appeal
viz_millennial <- ggplot(yimby_millennial, 
                         aes(x = avg_housing_growth, y = young_adult_pct)) +
  geom_point(aes(size = arts_emp_per_1000, color = rent_burden_change), alpha = 0.6) +
  ...
  labs(
    title = "YIMBY Cities Attract Young Adults",
    subtitle = "Higher housing growth correlates with larger young adult populations",
    x = "Housing Growth Score (higher = more building-friendly)",
    y = "Young Adults (Ages 25-34) as % of Population",
    caption = "Point size represents arts, entertainment & recreation employment per 1,000 residents\nColor shows change in rent burden (green = improvement)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
...
print(viz_millennial)

# Calculate correlation
correlation_result <- cor.test(yimby_millennial$avg_housing_growth, 
                                yimby_millennial$young_adult_pct, 
                                use = "complete.obs")

cat("\n\nCorrelation between Housing Growth and Young Adult Population:\n")
cat("Correlation coefficient:", round(correlation_result$estimate, 3), "\n")
cat("P-value:", format.pval(correlation_result$p.value, digits = 3), "\n")

# Identify YIMBY cities that are particularly attractive to young adults
yimby_millennial_winners <- yimby_millennial |>
  filter(!is.na(young_adult_pct), !is.na(arts_emp_per_1000)) |>
  mutate(
    millennial_score = (young_adult_pct / mean(young_adult_pct, na.rm = TRUE)) * 50 +
                       (arts_emp_per_1000 / mean(arts_emp_per_1000, na.rm = TRUE)) * 50
  ) |>
  filter(yimby_score >= 3, millennial_score > 75) |>
  arrange(desc(millennial_score))

cat("\n\nYIMBY Cities with High Millennial Appeal:\n")
cat("(Cities with YIMBY score >= 3 AND high young adult/arts employment)\n")
print(
  yimby_millennial_winners |>
    select(NAME, yimby_score, young_adult_pct, arts_emp_per_1000, 
           millennial_score, rent_burden_change) |>
    format_titles() |>
    datatable(options = list(searching = TRUE, paging = TRUE, pageLength = 10, info = FALSE, paging = FALSE)) |>
    formatRound(c("Yimby Score", "Millennial Score"), 0) |>
    formatRound(c("Young Adult Pct", "Arts Emp Per 1000", "Rent Burden Change"), 2)
)
```

### Millennial Appeal Analysis: Key Findings

The data reveals a strong positive relationship between YIMBY housing policies and millennial attraction. Cities with higher housing growth scores tend to have:

1. **Larger young adult populations (ages 25-34)**: The correlation between housing growth and young adult percentage is positive and statistically significant, suggesting that building-friendly policies help cities attract and retain millennials.

2. **More arts and entertainment jobs**: Cities with robust housing growth also tend to have thriving cultural sectors, with more arts, entertainment, and recreation employment per capita. This creates a virtuous cycle where housing affordability enables young creative professionals to live and work in these cities.

3. **Improving affordability**: YIMBY cities that successfully reduced rent burden while growing their housing stock have become magnets for young professionals who want urban amenities without crushing housing costs.

This matters for policy because:

- **Economic vitality**: Young adults drive innovation, entrepreneurship, and economic growth. Cities that lose young people to housing costs face long-term economic stagnation.

- **Fiscal sustainability**: A younger population provides a stronger tax base and reduces the dependency ratio, making cities more financially sustainable.

- **Cultural vibrancy**: Young adults support the arts, entertainment, and restaurant sectors that make cities attractive places to live and work.

- **Political coalition**: Young voters increasingly prioritize housing affordability. YIMBY policies that attract millennials also build political support for continued reform.

**For the Policy Brief**: These findings demonstrate that YIMBY policies don't just address housing affordability—they make cities more attractive to the young, educated workforce that drives 21st-century economic growth. Federal grants should prioritize cities that commit to both housing growth AND preserving/expanding the cultural amenities that make urban living appealing to young professionals.


```{r}
# Check the data
summary(yimby_millennial$avg_housing_growth)
summary(yimby_millennial$young_adult_pct)

# How many complete observations?
sum(complete.cases(yimby_millennial$avg_housing_growth, 
                   yimby_millennial$young_adult_pct))

# Look at the data
head(yimby_millennial |> 
     select(NAME, avg_housing_growth, young_adult_pct) |> 
     arrange(desc(avg_housing_growth))) |> 
      format_titles() |>
    datatable(options = list(searching = FALSE, info = FALSE, paging = FALSE)) |>
    formatRound(c("Avg Housing Growth"), 0) |> 
  formatRound(c("Young Adult Pct"), 0) 
```
